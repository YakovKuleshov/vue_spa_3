<template>
  <div class="chart__labels">
    <div
      ...
      :class="{
        chart__labels__item_active: item.selected,
    ...
    >
      <div
        ...
        :style="{
          backgroundColor: item.selected ? item.backgroundColor : 'transparent'
        }"
      ></div>
      <div class="chart__labels__label">{{ item.label }}</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    hideChart(item) {
      ...
      this.$emit('hideChart', item)
    },
    onePointLeft(item) {
      return this.chartData.filter((el) => (item.selected ? el.selected : [])).length === 1
    }
  }
}
</script>
